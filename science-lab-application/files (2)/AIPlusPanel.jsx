...
export default function AIPlusPanel({ onClose }) {
  ...
  const handleChat = async () => {
    if (!chatInput.trim() || !ai.apiKey) return;
  ...
  const labelStyle = { fontSize: 12, color: '#6b7280', display: 'block', marginBottom: 5, fontWeight: 500 };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* 좌측: 모드 선택 */}
        <div style={{ width: 160, background: '#f9fafb', borderRight: '0.5px solid #f0f0f0', padding: '20px 0', flexShrink: 0 }}>
          <div style={{ padding: '0 14px 16px', fontSize: 14, fontWeight: 700, color: '#1f2937' }}>
            ✦ AI Plus
          </div>
          <div style={{ fontSize: 11, color: '#9ca3af', padding: '0 14px 8px' }}>
            {ai.provider === 'claude' ? 'Claude' : ai.provider === 'openai' ? 'GPT-4o' : 'Gemini'}
          </div>
          {MODES.map(m => (
            <button key={m.id} onClick={() => { setMode(m.id); setResult(''); setError(''); }}
              style={{
                width: '100%', padding: '10px 14px', background: mode === m.id ? theme.accentColor + '15' : 'none',
                border: 'none', cursor: 'pointer', textAlign: 'left',
                borderLeft: `3px solid ${mode === m.id ? theme.accentColor : 'transparent'}`,
                transition: 'all 0.15s'
              }}>
              <div style={{ fontSize: 13, fontWeight: mode === m.id ? 600 : 400, color: mode === m.id ? theme.accentColor : '#374151' }}>{m.label}</div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '16px 24px' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  {chatHistory.map((m, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                      <div style={{
                        ...
                        background: m.role === 'user' ? theme.accentColor : '#f3f4f6',
                  ...
                  {loading && (
                    <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
                      <div style={{ background: '#f3f4f6', padding: '10px 16px', borderRadius: 12, fontSize: 13, color: '#9ca3af' }}>
                        생각하는 중...
                      </div>
                    </div>
                  )}
            ...
            {/* 오류 */}
            {error && (
              <div style={{ background: '#fee2e2', borderRadius: 10, padding: '10px 14px', marginTop: 12, fontSize: 13, color: '#991b1b' }}>
                ⚠️ {error}
              </div>
            )}
            ...
            {result && (
              <div ref={resultRef} style={{ marginTop: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: '#374151' }}>✦ AI 생성 결과</div>
                ...
                <div style={{
                  ...
                  fontSize: 13, color: '#1f2937', lineHeight: 1.8,
          ...
          {/* 하단 버튼 (채팅 제외) */}
          {mode !== 'chat' && (
            <div style={{ padding: '14px 24px', borderTop: '0.5px solid #f0f0f0' }}>
